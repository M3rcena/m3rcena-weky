...
import {
	checkPackageUpdates, createEmbed, getButtonDilemma, getRandomString
...
const WillYouPressTheButton = async (options: WillYouPressTheButtonTypes) => {
    ...
    if (!interaction) throw new Error(chalk.red("[@m3rcena/weky] WillYouPressTheButton Error:") + " No interaction provided.");

    if (!interaction.channel || !interaction.channel.isSendable()) throw new Error(chalk.red("[@m3rcena/weky] WillYouPressTheButton Error:") + " No channel provided in interaction.");
