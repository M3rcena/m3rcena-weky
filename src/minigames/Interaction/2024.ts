...
const mini2048 = async (options: Types2048) => {
    ...
    if (!interaction) throw new Error(chalk.red("[@m3rcena/weky] 2048 Error:") + " No interaction provided.");

    if (!interaction.isChatInputCommand()) throw new Error(chalk.red("[@m3rcena/weky] 2048 Error:") + " Interaction is not a ChatInputCommandInteraction.");

    let client: Client = options.client;

    if (!interaction.channel) throw new Error(chalk.red("[@m3rcena/weky] 2048 Error:") + " No channel found on Interaction.");

    if (!interaction.channel.isSendable()) throw new Error(chalk.red("[@m3rcena/weky] 2048 Error:") + " Channel is not sendable.");

    if (!interaction.guild) throw new Error(chalk.red("[@m3rcena/weky] 2048 Error:") + " No guild found on Interaction.");
    ...
    options.embed.description = originalDescription?.replace(
