...
import type { GuessThePokemonData, GuessThePokemonTypes } from "../../Types";
const gameData = new Set();

const GuessThePokemon = async (options: GuessThePokemonTypes) => {
    OptionsChecking(options, "GuessThePokemon");

    let interaction = options.interaction;

    if (!interaction) throw new Error(chalk.red("[@m3rcena/weky] GuessThePokemon Error:") + " No interaction provided.");

    if (!interaction.channel || !interaction.channel.isSendable()) throw new Error(chalk.red("[@m3rcena/weky] GuessThePokemon Error:") + " No channel found.");
    ...
    const data = await fetch(`http://pokeapi.co/api/v2/pokemon/${randomNumber}`).then(res => res.json()) as GuessThePokemonData;
