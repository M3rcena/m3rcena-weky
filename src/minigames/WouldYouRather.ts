...
const WouldYouRather = async (options: WouldYouRatherTypes) => {
	...
	const res = {
		...
		percentage: {
			1:
				((parseInt(data.option1Votes) / (parseInt(data.option1Votes) + parseInt(data.option2Votes))) * 100).toFixed(2) +
				"%",
			2:
				((parseInt(data.option2Votes) / (parseInt(data.option1Votes) + parseInt(data.option2Votes))) * 100).toFixed(2) +
				"%",
		},
	};
	...
	gameCollector.on("collect", async (wyr) => {
		if (wyr.user.id !== id) {
			return wyr.reply({
				content: options.othersMessage ? options.othersMessage.replace("{{author}}", id) : `This is not your game!`,
