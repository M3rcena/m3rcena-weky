...
export default class WouldYouRather implements IMinigame {
	...
	private options: CustomOptions<WouldYouRatherTypes>;
	private context: CustomOptions<WouldYouRatherTypes>["context"];
	...
	public async start() {
		...
		try {
			...
			const apiResponse = await ofetch<WYRApiResponse>(
				`https://io.wyr.app/api/v1/statements/en/easy?pageOffset=${pageOffset}&pageSize=1`,
	 ...
	 * @private
	 */
	private async endGame(state: "result" | "timeout" | "error", data?: { userChoice?: "A" | "B" }) {
	 ...
	 * @private
	 */
	private capitalizeFirstLetter(val: string | undefined): string {
		if (!val) return "";
	...
	private createGameContainer(
		state: "loading" | "active" | "result" | "timeout" | "error",
		data: {
			option1?: string;
			option2?: string;
			stats?: { a: string; b: string };
			userChoice?: "A" | "B";
		}
	): ContainerBuilder {
