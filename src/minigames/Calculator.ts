...
const Calculator = async (client: Client, options: CalcTypes, loggerManager: LoggerManager) => {
	...
	calc.on("collect", async (interact) => {
			...
			return interact.reply({
				embeds: [
					new EmbedBuilder()
						...
						.setDescription(
							`You are not allowed to interact with this calculator as you are not the user who initiated the command.\n\n**Note:** This calculator is only for the user <@${id}>`
						)
						.setColor("Red")
		...
		if (disabled === true && lastInput !== null && lastInput !== undefined) {
			...
			((disabled === false && lastInput === null) || lastInput === undefined) &&
