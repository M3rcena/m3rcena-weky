...
import { evaluate, format, type Matrix, type Unit, type Complex, type BigNumber } from "mathjs";
...
const Calculator = async (weky: WekyManager, options: CustomOptions<CalcTypes>) => {
	...
	const MODAL_OPERATIONS: Record<string, ModalOpConfig> = {
		calLG: {
	...
	const calculateResult = (input: string): CalculationResult => {
		try {
	...
	const buildRows = (keys: string[], forceEmpty: boolean = false) => {
	...
	const handleModal = async (interaction: any, config: ModalOpConfig): Promise<string | null> => {
		...
		const textDisplay = new TextDisplayBuilder().setContent(
			options.modals?.display
				? options.modals.display.replace(
						"{{currentExpression}}",
						currentExpression === " "
		...
		try {
			const submitted = await interaction.awaitModalSubmit({
				filter: (i: ModalSubmitInteraction) => i.customId === `md${modalId}` && i.user.id === authorId,
				time: 300_000,
			});
	...
	const updateUI = async (forceEmpty: boolean = false) => {
		const p1 = msg1.edit({
	...
	collector.on("collect", async (interaction) => {
		...
		switch (true) {
			...
			case id === "cal⌫":
				if (!currentExpression || currentExpression === " ") break;
