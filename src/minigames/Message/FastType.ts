...
const FastType = async (options: FastTypeTypes) => {
    OptionsChecking(options, "FastType")

    let message = options.message;

    if (!message) throw new Error(chalk.red("[@m3rcena/weky] FastType Error:") + " No message provided.");

    if (!message.channel) throw new Error(chalk.red("[@m3rcena/weky] FastType Error:") + " No channel found on Message.");

    if (!message.channel.isSendable()) throw new Error(chalk.red("[@m3rcena/weky] FastType Error:") + " Channel is not sendable.");

    if (message.channel.isDMBased()) throw new Error(chalk.red("[@m3rcena/weky] FastType Error:") + " Channel is a DM.");
