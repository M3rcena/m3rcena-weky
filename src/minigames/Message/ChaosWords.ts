...
import type { ChaosTypes, Fields } from "../../Types";

const data = new Set();

const ChaosWords = async (options: ChaosTypes) => {
    // Check types
    OptionsChecking(options, "ChaosWords")

    let message = options.message;

    if (!message) throw new Error(chalk.red("[@m3rcena/weky] ChaosWords Error:") + " No message provided.");

    if (!message.channel) throw new Error(chalk.red("[@m3rcena/weky] ChaosWords Error:") + " No channel found on Message.");

    if (!message.channel.isSendable()) throw new Error(chalk.red("[@m3rcena/weky] ChaosWords Error:") + " Channel is not sendable.");

    if (message.channel.isDMBased()) throw new Error(chalk.red("[@m3rcena/weky] ChaosWords Error:") + " Channel is a DM.");
    ...
    const game = message.channel.createMessageCollector({
