...
const WillYouPressTheButton = async (options: WillYouPressTheButtonTypes) => {
    ...
    if (!message) throw new Error(chalk.red("[@m3rcena/weky] WillYouPressTheButton Error:") + " No message provided.");

    if (!message.channel || !message.channel.isSendable()) throw new Error(chalk.red("[@m3rcena/weky] WillYouPressTheButton Error:") + " No channel provided in message.");
