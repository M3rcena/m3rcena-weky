...
const Fight = async (options: FightTypes) => {
    ...
    if (!message) throw new Error(chalk.red("[@m3rcena/weky] Fight Error:") + " No message provided.");

    if (!message.channel.isSendable()) throw new Error(chalk.red("[@m3rcena/weky] Fight Error:") + " Channel is not sendable.");

    if (message.channel.isDMBased()) throw new Error(chalk.red("[@m3rcena/weky] Fight Error:") + " Channel is a DM.");
