...
const GuessTheNumber = async (options: GuessTheNumberTypes) => {
    ...
    if (message.channel.isDMBased()) {
        throw new Error(chalk.red("[@m3rcena/weky] GuessTheNumber Error:") + " Channel is a DM.");
    };
    ...
    const handleGame = async (isPublic: boolean) => {
        ...
        collector.on('collect', async (_msg) => {
            ...
            if (parsedNumber === number) {
                ...
                if (options.returnWinner) {
                    ...
                    db.set(
                        `GuessTheNumber_${message.guild.id}_${options.gameID}`,
                        _msg.author.id
                    );
                }
            }
        ...
        collector.on('end', async (_collected, reason) => {
            if (reason === 'time') {
                ...
                if (!message.channel || !message.channel.isSendable()) return;
