...
import type { SnakeTypes } from "../Types/index.js";
...
const Snake = async (options: SnakeTypes) => {
	...
	if (!interaction.guild) {
		throw new Error(chalk.red("[@m3rcena/weky] Snake Error:") + " Guild is not available in this message.");
	}

	if (!interaction.channel || !interaction.channel.isSendable()) {
		throw new Error(chalk.red("[@m3rcena/weky] Snake Error:") + " Channel is not available in this message.");
	}
	...
	options.emojis = Object.fromEntries(
		Object.entries(defaultEmojis).map(([key, val]) => [
			key,
			options.emojis && options.emojis[key] ? options.emojis[key] : val,
		])
	) as typeof defaultEmojis;
	...
	options.snake = { ...defaultSnake, ...(options.snake || {}) };

	for (let y = 0; y < 10; y++) {
		for (let x = 0; x < 15; x++) {
			gameBoard[y * 15 + x] = options.emojis.board || "⬛";
		}
	}

	function getBoardContent(isSkull: boolean) {
		const emojis = options.snake
			? options.snake
			: {
					head: "🟢",
					body: "🟩",
	...
	let baseDescription = options.embed.description || `**Score:** {{score}}\n\n{{board}}`;

	options.embed.title = options.embed.title ?? "Snake Game";
	options.embed.description =
		baseDescription.replace("{{score}}", score.toString()).replace("{{board}}", getBoardContent(false)) ??
		`**Score:** ${score}\n\n${getBoardContent(false)}`;
	let embed = createEmbed(options.embed);

	const up = new ButtonBuilder().setEmoji(emojis.up).setStyle(ButtonStyle.Primary).setCustomId("weky-snake_up");

	const down = new ButtonBuilder().setEmoji(emojis.down).setStyle(ButtonStyle.Primary).setCustomId("weky-snake_down");

	const left = new ButtonBuilder().setEmoji(emojis.left).setStyle(ButtonStyle.Primary).setCustomId("weky-snake_left");
	...
	const stop = new ButtonBuilder().setEmoji(emojis.stop).setStyle(ButtonStyle.Danger).setCustomId("weky-snake_stop");
	...
	collector.on("collect", async (btn) => {
		...
		if (isSnake(nextPos) || buttonId === "stop") {
			...
			options.embed.description = baseDescription
				? baseDescription.replace("{{score}}", `${score}`).replace("{{board}}", `${newBoardContent}`)
				: `**Score:** ${score}\n\n${newBoardContent}`;
	...
	collector.on("end", async (_, reason) => {
		if (reason === "time" || reason === "user") {
			options.embed.description =
				baseDescription.replace("{{score}}", score.toString()).replace("{{board}}", getBoardContent(true)) ??
