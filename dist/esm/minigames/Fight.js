import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ContainerBuilder, MediaGalleryBuilder, MessageFlags, } from "discord.js";
...
export default class Fight {
    ...
    async handleHit(interaction) {
        ...
        if (!updated)
            return interaction.followUp({ content: "API Error", flags: [MessageFlags.Ephemeral] });
        if (enemy.health <= 0) {
    ...
    createGameContainer(state, details) {
        ...
        if (state === "request") {
            const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel(this.btnAccept).setStyle(ButtonStyle.Success).setCustomId("fight_accept"), new ButtonBuilder().setLabel(this.btnDeny).setStyle(ButtonStyle.Danger).setCustomId("fight_deny"));
            container.addActionRowComponents(row);
        }
        else if (state === "active") {
            const mainRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel(this.btnHit).setStyle(ButtonStyle.Danger).setCustomId("fight_hit"), new ButtonBuilder().setLabel(this.btnHeal).setStyle(ButtonStyle.Success).setCustomId("fight_heal"), new ButtonBuilder().setLabel(this.btnCancel).setStyle(ButtonStyle.Secondary).setCustomId("fight_surrender"));
            container.addActionRowComponents(mainRow);
            const powerupRow = new ActionRowBuilder().addComponents(this.POWERUPS.map((p) => new ButtonBuilder().setLabel(`${p.label} (${p.cost}🪙)`).setStyle(p.style).setCustomId(`powerup_${p.id}`)));
