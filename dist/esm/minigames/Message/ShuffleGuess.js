...
import { checkPackageUpdates, convertTime, createEmbed, getRandomSentence, getRandomString, shuffleString } from "../../functions/functions.js";
...
const ShuffleGuess = async (options) => {
    ...
    if (!message.channel || !message.channel.isSendable())
        throw new Error(chalk.red("[@m3rcena/weky] FastType Error:") + " No channel provided in message.");
    ...
    const embed = await message.reply({
        embeds: [emd],
        components: [new ActionRowBuilder().addComponents(disbut, cancel)],
    });
    ...
    gameCollector.on('end', async (_collected, reason) => {
        if (reason === 'time') {
            ...
            options.embed.description = options.loseMessage ?
                ...
                `The word was \`${options.word.toString()}\``;
            let _embed = createEmbed(options.embed);
            await message.reply({ embeds: [_embed] });
            data.delete(id);
            return embed.edit({
                embeds: [emd],
                components: [new ActionRowBuilder().addComponents(disbut, cancel)],
