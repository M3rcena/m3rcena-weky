...
class ShuffleGuess {
    ...
    async start() {
        ...
        if (isNaN(this.options.time) || this.options.time < 10000) {
            this.endGame("lost");
            return this.weky._LoggerManager.createError("ShuffleGuess", "time must be greater than 10 Seconds (in ms i.e. 10000)");
        }
     ...
     * @private
     */
    createGameContainer(state, scrambledWord, feedback = "") {
        let color = 0x5865f2;
        if (state === "correct")
            color = 0x57f287;
        if (state === "wrong" || state === "lost")
            color = 0xed4245;
        if (typeof this.options.embed.color === "number" && state === "playing")
            color = this.defaultColor;
        const container = new discord_js_1.ContainerBuilder().setAccentColor(color);
        let mainContent = this.gameTitle ? `## ${this.gameTitle}\n` : `## Shuffle Guess\n`;
        if (state === "playing" || state === "wrong") {
            const timeStr = this.weky.convertTime(this.options.time);
        ...
        const btnReshuffle = new discord_js_1.ButtonBuilder()
