...
const Calculator = async (weky, options) => {
    ...
    const handleModalInput = async (interact, modalId, operation) => {
        ...
        const input = new discord_js_1.TextInputBuilder()
            ...
            .setRequired(true);
        const label = new discord_js_1.LabelBuilder().setLabel(`Enter the number for ${operation}`).setTextInputComponent(input);
        const text = new discord_js_1.TextDisplayBuilder().setContent(`## Current Calculation Prompt:\n${str === " " ? "```No Prompt Yet```" : stringify}`);
    ...
    const createCalculatorContainer2 = (buttonsEnabled, lock) => {
        const container = new discord_js_1.ContainerBuilder().setAccentColor(typeof options.embed.color === "number" ? options.embed.color : 0x5865f2);
    ...
    calc.on("collect", async (interact) => {
        ...
        switch (interact.customId) {
            ...
            case "cal=":
                ...
                if (result !== null) {
                    answer = (0, mathjs_1.format)(result, { precision: 14 });
                    str += " = " + result;
                }
                else {
                    str = error;
