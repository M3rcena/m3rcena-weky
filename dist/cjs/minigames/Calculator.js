...
class Calculator {
    ...
    async onInteraction(interaction) {
        ...
        switch (true) {
            ...
            case id === "cal=":
                ...
                if (result !== null) {
                    this.lastAnswer = (0, mathjs_1.format)(result, { precision: 14 });
     ...
     * @private
     */
    async updateUI(forceEmpty = false) {
        if (!this.msg1)
            return;
        ...
        const p2 = this.msg2 && this.msg2.editable
            ? this.msg2.edit({
    ...
    async handleModal(interaction, config) {
        ...
        const modal = new discord_js_1.ModalBuilder().setTitle(modalId).setCustomId(modalCustomId);
        const input = new discord_js_1.TextInputBuilder()
            .setCustomId(`input${modalId}`)
            .setStyle(discord_js_1.TextInputStyle.Short)
            .setRequired(true);
        const textDisplay = new discord_js_1.TextDisplayBuilder().setContent(this.options.modals?.display
            ? this.options.modals.display.replace("{{currentExpression}}", this.currentExpression === " "
        ...
        modal
            ...
            .setLabel(this.options.modals?.labels
        ...
        try {
            const submitted = await interaction.awaitModalSubmit({
                filter: (i) => i.customId === modalCustomId && i.user.id === this.id,
                time: 300_000,
            });
     ...
     * @private
     */
    buildRows(keys, forceEmpty = false) {
        const container = new discord_js_1.ContainerBuilder().setAccentColor(this.accentColor);
