...
class mini2048 {
    ...
    async onInteraction(interaction) {
        ...
        if (!moveResult) {
            return interaction.followUp({ content: "Connection error!", flags: [discord_js_1.MessageFlags.Ephemeral] });
        }
    ...
    createGameContainer(state, details) {
        ...
        if (state === "active") {
            const up = new discord_js_1.ButtonBuilder().setStyle(discord_js_1.ButtonStyle.Secondary).setLabel(this.emojiUp).setCustomId("weky_2048_up");
