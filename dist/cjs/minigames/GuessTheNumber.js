...
const activePrivatePlayers = new Set();
const currentPublicGames = {};
/**
...
class GuessTheNumber {
    ...
    async start() {
        if (this.isPublic && currentPublicGames[this.context.guild.id]) {
            ...
            const msgTemplate = this.options.ongoingMessage || "A game is already running in <#{{channel}}>. Try again later!";
            ...
            return this.context.channel.send({ components: [errorContainer], flags: discord_js_1.MessageFlags.IsComponentsV2 });
        }
        if (!this.isPublic) {
            if (activePrivatePlayers.has(this.id))
                return;
    ...
    async onMessage(message) {
        ...
        if (guess < this.targetNumber) {
            state = "higher";
            if (guess > this.currentMin)
                this.currentMin = guess;
        }
        else {
            state = "lower";
            if (guess < this.currentMax)
                this.currentMax = guess;
        }
    ...
    createGameContainer(state, data) {
        ...
        switch (state) {
            ...
            case "lost":
                ...
                const loseText = (this.options.loseMessage || "The number was **{{number}}**.").replace("{{number}}", `${this.targetNumber}`);
