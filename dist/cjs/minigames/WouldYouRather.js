...
class WouldYouRather {
    ...
    async start() {
        ...
        try {
            ...
            const apiResponse = await (0, ofetch_1.ofetch)(`https://io.wyr.app/api/v1/statements/en/easy?pageOffset=${pageOffset}&pageSize=1`, { timeout: 5000, retry: 1 });
    ...
    createGameContainer(state, data) {
        ...
        if (state === "active" || state === "result") {
            ...
            const btnA = new discord_js_1.ButtonBuilder().setStyle(styleA).setLabel(txtA).setCustomId(this.idA).setDisabled(isResult);
            const btnB = new discord_js_1.ButtonBuilder().setStyle(styleB).setLabel(txtB).setCustomId(this.idB).setDisabled(isResult);
