...
async function getRequestCard(challenger, opponent) {
    ...
    const canvas = new canvas_1.Canvas(800, 250);
    ...
    const [challengerAvatar, opponentAvatar] = await Promise.all([
        (0, canvas_1.loadImage)(challenger.displayAvatarURL({ extension: 'png', size: 256 })),
        (0, canvas_1.loadImage)(opponent.displayAvatarURL({ extension: 'png', size: 256 }))
    ]);
    ...
    return new discord_js_1.AttachmentBuilder(canvas.toBuffer("image/png"), { name: 'fight-request.png' });
}
async function getMainCard(player1, player2) {
    const canvas = new canvas_1.Canvas(1000, 300);
    const ctx = canvas.getContext('2d');
    // Set background
    ...
    const [avatar1, avatar2] = await Promise.all([
        (0, canvas_1.loadImage)(player1.member.displayAvatarURL({ extension: 'png', size: 256 })),
        (0, canvas_1.loadImage)(player2.member.displayAvatarURL({ extension: 'png', size: 256 }))
    ]);
    ...
    return new discord_js_1.AttachmentBuilder(canvas.toBuffer("image/png"), { name: 'fight-status.png' });
}
