...
const Snake = async (options) => {
    ...
    if (!message)
        throw new Error(chalk_1.default.red("[@m3rcena/weky] Snake Error:") + " No message provided.");
    let id = message.author.id;
    if (!message.guild) {
        throw new Error(chalk_1.default.red("[@m3rcena/weky] Snake Error:") + " Guild is not available in this message.");
    }
    ;
    if (!message.channel || !message.channel.isSendable()) {
        throw new Error(chalk_1.default.red("[@m3rcena/weky] Snake Error:") + " Channel is not available in this message.");
    }
    ;
    const snake = [{ x: 5, y: 5 }];
    let apple = { x: 1, y: 1 };
    ...
    options.emojis = Object.fromEntries(Object.entries(defaultEmojis).map(([key, val]) => [
        key,
