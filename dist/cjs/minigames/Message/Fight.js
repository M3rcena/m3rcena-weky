...
const fightCards_js_1 = require("../../functions/fightCards.js");
...
const Fight = async (options) => {
    ...
    if (!message)
        throw new Error(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " No message provided.");
    if (!message.channel.isSendable())
        throw new Error(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Channel is not sendable.");
    if (message.channel.isDMBased())
        throw new Error(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Channel is a DM.");
    ...
    if (typeof options.buttons !== "object") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Buttons must be an object.");
    }
    ;
    if (!options.buttons.hit)
        options.buttons.hit = "Hit";
    if (typeof options.buttons.hit !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Hit button text must be a string.");
    }
    ;
    if (!options.buttons.heal)
        options.buttons.heal = "Heal";
    if (typeof options.buttons.heal !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Heal button text must be a string.");
    }
    ;
    if (!options.buttons.cancel)
        options.buttons.cancel = "Surrender";
    if (typeof options.buttons.cancel !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Cancel button text must be a string.");
    }
    ;
    if (!options.buttons.accept)
        options.buttons.accept = "Accept";
    if (typeof options.buttons.accept !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Accept button text must be a string.");
    }
    ;
    if (!options.buttons.deny)
        options.buttons.deny = "Deny";
    if (typeof options.buttons.deny !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Deny button text must be a string.");
    }
    ;
    ...
    if (typeof options.acceptMessage !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Accept message must be a string.");
    }
    ;
    if (!options.winMessage)
        options.winMessage = "GG, <@{{winner}}> won the fight!";
    if (typeof options.winMessage !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Win message must be a string.");
    }
    ;
    ...
    if (typeof options.endMessage !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " End message must be a string.");
    }
    ;
    if (!options.cancelMessage)
        options.cancelMessage = "<@{{opponent}}> refused to have a fight with you!";
    if (typeof options.cancelMessage !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Cancel message must be a string.");
    }
    ;
    if (!options.fightMessage)
        options.fightMessage = "{{player}} you go first!";
    if (typeof options.fightMessage !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Fight message must be a string.");
    }
    ;
    if (!options.othersMessage)
        options.othersMessage = "Only {{author}} can use the buttons!";
    if (typeof options.othersMessage !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Others message must be a string.");
    }
    ;
    ...
    if (typeof options.opponentsTurnMessage !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Opponents turn message must be a string.");
    }
    ;
    ...
    if (typeof options.highHealthMessage !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " High health message must be a string.");
    }
    ;
    ...
    if (typeof options.lowHealthMessage !== "string") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Low health message must be a string.");
    }
    ;
    if (!options.returnWinner)
        options.returnWinner = false;
    if (typeof options.returnWinner !== "boolean") {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Return winner must be a boolean.");
    }
    ;
    if (!options.opponent) {
        throw new Error(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Opponent is missing from the options.");
    }
    ;
    if (!options.opponent.username) {
        throw new TypeError(chalk_1.default.red("[@m3rcena/weky] Fight Error:") + " Opponent option must be User.");
    }
    ...
    const requestCard = await (0, fightCards_js_1.getRequestCard)(challenger, opponent);
    ...
    const row = new discord_js_1.ActionRowBuilder().addComponents(accept, deny);
    ...
    const collector = msg.createMessageComponentCollector({ filter, time: 30000, componentType: discord_js_1.ComponentType.Button });
    collector.on('collect', async (interaction) => {
        if (interaction.customId === 'fight_deny') {
            const deniedCard = await (0, fightCards_js_1.getDeniedCard)(challenger, opponent);
    ...
    collector.on('end', async (collected, reason) => {
        if (reason === 'time' && collected.size === 0) {
            const timeoutCard = await (0, fightCards_js_1.getTimeoutCard)(challenger, opponent);
...
async function startFight(interaction, options, player1, player2) {
    ...
    const hit = new discord_js_1.ButtonBuilder()
        .setLabel(options.buttons.hit)
        .setStyle(discord_js_1.ButtonStyle.Danger)
        .setCustomId('fight_hit');
    const heal = new discord_js_1.ButtonBuilder()
        .setLabel(options.buttons.heal)
        .setStyle(discord_js_1.ButtonStyle.Success)
        .setCustomId('fight_heal');
    const surrender = new discord_js_1.ButtonBuilder()
        .setLabel(options.buttons.cancel)
        .setStyle(discord_js_1.ButtonStyle.Secondary)
        .setCustomId('fight_surrender');
    // Create powerup buttons
    const powerupButtons = POWERUPS.map(powerup => new discord_js_1.ButtonBuilder()
        .setLabel(`${powerup.label} (${powerup.cost}🪙)`)
        .setStyle(powerup.style)
        .setCustomId(`powerup_${powerup.id}`));
    const actionRow = new discord_js_1.ActionRowBuilder().addComponents(hit, heal, surrender);
    const powerupRow = new discord_js_1.ActionRowBuilder().addComponents(...powerupButtons);
    // Update the fight status
    const gameCard = await (0, fightCards_js_1.getMainCard)(player1, player2);
    ...
    collector.on('collect', async (i) => {
        ...
        if (i.customId === 'fight_hit') {
            ...
            const surrenderCard = await (0, fightCards_js_1.getSurrenderCard)(currentPlayer.member, opponent.member);
            const surrenderEmbed = new discord_js_1.EmbedBuilder()
                .setColor('Red')
        ...
        // Update game state
        const newGameCard = await (0, fightCards_js_1.getMainCard)(player1, player2);
        const newGameEmbed = new discord_js_1.EmbedBuilder()
            .setColor(options.embed.color ?? "Blurple")
            .setImage("attachment://fight-status.png")
            .setDescription(`It's ${currentPlayer.member.username}'s turn!`);
        await i.editReply({
