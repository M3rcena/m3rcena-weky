...
class QuickClick {
    ...
    async start() {
        ...
        this.buttons = [];
        for (let i = 0; i < 25; i++) {
            this.buttons.push(new discord_js_1.ButtonBuilder()
                .setLabel("\u200b")
    ...
    async endGame(state, data) {
        ...
        if (state === "won") {
            this.buttons[this.winningIndex].setDisabled(true).setStyle(discord_js_1.ButtonStyle.Success);
        }
        else {
            this.buttons[this.winningIndex].setDisabled(true).setStyle(discord_js_1.ButtonStyle.Secondary);
        }
        if (this.gameMessage) {
            try {
                await this.gameMessage.edit({
