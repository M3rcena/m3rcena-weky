...
exports.getRequestCard = getRequestCard;
exports.getMainCard = getMainCard;
exports.getSurrenderCard = getSurrenderCard;
exports.getDeniedCard = getDeniedCard;
exports.getTimeoutCard = getTimeoutCard;
const discord_js_1 = require("discord.js");
const canvas_1 = require("@napi-rs/canvas");
// Request Card
async function getRequestCard(challenger, opponent) {
    const canvas = new canvas_1.Canvas(800, 800); // Square aspect ratio
...
// Surrender Card
async function getSurrenderCard(player, winner) {
    const canvas = new canvas_1.Canvas(800, 800);
    const ctx = canvas.getContext('2d');
    // Background
    const gradient = ctx.createLinearGradient(0, 0, 800, 800);
    ...
    const [playerAvatar, winnerAvatar] = await Promise.all([
        (0, canvas_1.loadImage)(player.displayAvatarURL({ extension: 'png', size: 256 })),
        (0, canvas_1.loadImage)(winner.displayAvatarURL({ extension: 'png', size: 256 }))
    ]);
    ctx.save();
    ctx.beginPath();
...
// Denied Card
async function getDeniedCard(challenger, opponent) {
    const canvas = new canvas_1.Canvas(800, 800);
    const ctx = canvas.getContext('2d');
    // Background
    const gradient = ctx.createLinearGradient(0, 0, 800, 800);
    ...
    return new discord_js_1.AttachmentBuilder(canvas.toBuffer('image/png'), { name: 'fight-denied.png' });
}
// Timeout Card
async function getTimeoutCard(challenger, opponent) {
    const canvas = new canvas_1.Canvas(800, 800);
    const ctx = canvas.getContext('2d');
    // Background
    const gradient = ctx.createLinearGradient(0, 0, 800, 800);
